<?php 
	$this->assign('title','Create Beverage & Bar Orders (BOT)');
	$this->assign('heading','Create Beverage & Bar Orders (BOT)');
	$this->assign('breadcrumb','Create Beverage & Bar Orders (BOT)'); 
...
<!--
	...
	$(function(){
		...
		function calcSum() {
			...
			$("#beverage-item-price").val(parseFloat(ramt).toFixed(2));
	...
	function find_item(cat_val,val)
	{
		...
		$.ajax({		
			type:'POST',
			url:"<?=$this->Url->build(['controller'=>'BeverageItemOrders', 'action'=>'fetchItem'])?>",
			data:{
				cat_id:cat_id[0]
			},
			dataType:'json',
			...
			beforeSend: function() {
				$("#beverage_item_name"+val).html('<option value="">Select Item</option>');
			},
			success:function(response){
				...
				if(response.item)
				{
					...
					$("#beverage_item_name"+val).html(item_html);
				}				
			},
	...
	/*function show_item_amount(item_val,val)
	{
		...
		$.ajax({		
			type:'POST',
			url:"<?php //echo $this->Url->build(['controller'=>'BeverageItemOrders', 'action'=>'fetchItemAmount'])?>",
			data:{
				item_id:item_id[0]
			},
			headers : {
...
<div id="tab-general">
	<div class="row mbl">
		...
		<?php echo $this->Form->create($beverageItemOrder, ["name" => "add-form", "id" => "add-form",  "method" => "POST", 'enctype'=>'multipart/form-data']); ?>
		<div class="col-lg-4">
			<div class="panel panel-grey">
				<div class="panel-heading">Beverage & Bar Order <?php echo $bookied_id!=0 ? 'for Booking Id: '.$bookied_id : '' ?></div>
										...
										<div class="col-md-12">
											<div class="form-group">
												...
												<div class="input-icon right">
													...
													{
														$booking_order_type = array('T' => 'Table', 'E' => 'Conference / Event');
														echo $this->Form->control('order_type', ['type'=>'select', 'options' => $booking_order_type, 'class' => 'form-control validate[required]', 'value' => $table, 'label'=>false, 'required'=>false, 'div'=>false, 'onchange' => 'show_room_table(this.value)']);
													}
													?>
												</div>
										...
										<div id="show_table">
											<div class="col-md-12">
												<div class="form-group">
													...
													<div class="input-icon right">
														<?php echo $this->Form->control('table_number', ['autocomplete' => 'off', 'placeholder' => 'Invoice No', 'class' => 'form-control validate[required]', 'data-errormessage-value-missing'=>'Invoice No is required', 'value' => 'BOT'."/".date("Y/m/d")."/".rand(1000, 9999), 'label'=>false, 'required'=>false, 'div'=>false]);?>
													</div>
												</div>
											</div>
											<div class="col-md-12">
												...
												<div class="input-icon right">
													<div class="form-group col-md-12" style="padding:0px" id="wrap">
														<table width="100%" border="1">
															...
															<tr>
																...
																<!-- <td>
																	<div class="input-icon right">
																		<select style="width:100%; border:0;" name="beverage_category_id[]" id="beverage_category_id1" class="form-control validate[required]" onChange="find_item(this.value, 1)" required>
																			<option value="">Select Item Category</option>
																			<?php 
																			  /*if($beveragecategories){
																				foreach($beveragecategories as $key=>$val){*/
																			  ?>
																			  <option value="<?php //echo $val->id."@".$val->beverage_item_name ?>"><?php //echo $val->beverage_item_name; ?></option>
																			  <?php 
																				/*}
																			  }*/
																			  ?>
																...
																<td>
																	<div class="input-icon right">
																		<select style="width:270px; border:0;" name="beverage_item_name[]" id="beverage_item_name1" class="chosen form-control validate[required]" onChange="show_item_amount(this.value, 1)" required>
																			<option value="">Select Item</option>
																			<?php 
																			  if($beverageItems){
																				foreach($beverageItems as $key=>$val){
																			  ?>
																			  <option value="<?php echo $val->beverage_item_name."@".$val->beverage_item_price ?>"><?php echo $val->beverage_item_name." - ".$val->beverage_item_price; ?></option>
																			  <?php 
																				}
																			  }
																			  ?>
													...
													<div class="col-md-4">
														<div class="form-group">
															...
															<div class="input-icon right">
																<?php echo $this->Form->control('beverage_item_price', ['autocomplete' => 'off', 'placeholder' => 'Item Price', 'class' => 'form-control validate[required]', 'data-errormessage-value-missing'=>'Item Price is required', 'type' => 'text', 'style' => 'background: #FFFFFF;', 'label'=>false, 'required'=>false, 'div'=>false, 'readonly' => true]);?>
															</div>
														</div>
													</div>													
													<div class="col-md-4">
...
$(".addmore").on('click',function(){
	...
	//data +="<td><div class='input-icon right'><select style='width:100%;border:0;' name='beverage_category_id[]' id='beverage_category_id"+i+"' class='form-control validate[required]' onChange='find_item(this.value, "+i+")' required><option value=''>Select Item Category</option><?php //if($beveragecategories){foreach($beveragecategories as $key=>$val){?><option value='<?php //echo $val->id.'@'.$val->beverage_item_name ?>'><?php //echo $val->beverage_item_name; ?></option><?php //}}?></select></div></td>";
	data +="<td><div class='input-icon right'><select style='width:270px;border:0;' name='beverage_item_name[]' id='beverage_item_name"+i+"' class='chosen form-control validate[required]' onChange='show_item_amount(this.value, "+i+")' required><option value=''>Select Item</option><?php if($beverageItems){foreach($beverageItems as $key=>$val){?><option value='<?php echo $val->beverage_item_name.'@'.$val->beverage_item_price ?>'><?php echo $val->beverage_item_name.' - '.$val->beverage_item_price; ?></option><?php }}?></select></div></td>";
