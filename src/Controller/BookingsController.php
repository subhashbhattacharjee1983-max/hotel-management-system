...
class BookingsController extends AppController
{
	...
	public function verify($booking_id = null, $room_id = null, $verified = null)
	{
		...
		$room_verify = $this->Rooms->find('all', ['conditions' => ['Rooms.id' => $room_id, 'Rooms.booking_id' => $booking_id, 'Rooms.room_status' => "O"]])->first();
		...
		if(!empty($room_verify))
		{
			...
			$this->Rooms->updateAll(['room_status' => "A", 'booking_id' => "0"], ['booking_id' => $booking_id, 'id' => $room_id]);
	...
	public function customerreport()
	{
		...
		if(isset($customer_id) && $customer_id!="")
		{
			$condition['Bookings.customer_id'] = $customer_id;
		}
		$this->Bookings = TableRegistry::get('Bookings');
		try {
			$this->paginate = [
	...
	public function showBooking()
	{
		...
		if(isset($booking_id) && $booking_id!=""){
			...
			if(!empty($booking))
			{
				...
				$data['bank_transfer_charge'] = $setting->currency.round($bank_transfer_charge);
				...
				$verify = $site_url."bookings/dashboard/".base64_encode($booking_id)."/".base64_encode($room_id);
	...
	public function dashboard($booking_id = null, $room_id = null){
		$this->loadModel('Rooms');
		...
		$this->set(compact('rooms', 'booking_reserve', 'reservation_booking_reserve', 'booking_id', 'room_id', 'booking_verify', 'room_verify'));
	}
    ...
    public function add($room_id = null)
    {
        ...
        if ($this->request->is('post')) {
			...
			$booking->bank_transfer_charge = $bank_transfer_charge;
    ...
    public function edit($id = null, $slug = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
			...
			if ($booking_data) {
                ...
                if($booking->booking_type == "B"){
					...
					$this->Flash->success(__('The reservation has been updated successfully'));
					return $this->redirect(['action' => 'reservation']);
				}
            }
            $this->Flash->error(__('The booking could not be saved. Please, try again.'));
        ...
        $roomCategories = $this->RoomCategories->find('all', ['conditions'=>['RoomCategories.status'=>'Y']])->toArray();
        $customers = $this->Bookings->Customers->find('list', ['limit' => 200]);
        $this->set(compact('booking', 'rooms', 'customers', 'roomCategories', 'slug'));
    }
	...
	public function customerbillpdf()
	{
		...
		$start_date = $this->request->getQuery('start_date_customer');
		$end_date = $this->request->getQuery('end_date_customer');
		$customer_id = $this->request->getQuery('customer_id');
		$condition['Bookings.payment_status IN'] = ["P","R"];
		if(isset($start_date) && $start_date!="")
		{
	...
	public function masterbillcsv($start_date = null, $end_date = null)
    {
		...
		$setting = $this->Settings->find('all', ['conditions' => []])->first();
		$data_array = array();
		$data_array_heading = "Booking Number<~@~>Customer Name<~@~>Mobile Number<~@~>Guest Category<~@~>Address<~@~>ID Type<~@~>ID Number<~@~>Total Room Charge<~@~>Total Food Charge<~@~>Total Bar Charge<~@~>Total House Keeping Charge<~@~>Gross Amount<~@~>Total Service Charge<~@~>Total BST Charge<~@~>Total GST Charge<~@~>Total Bank Transfer Charge<~@~>Grand Total";
	...
	public function roombillcsv($start_date = null, $end_date = null)
    {
		...
		$setting = $this->Settings->find('all', ['conditions' => []])->first();
		$data_array = array();
		$data_array_heading = "Booking Number<~@~>Room Number<~@~>Customer Name<~@~>Mobile Number<~@~>Guest Category<~@~>Address<~@~>ID Type<~@~>ID Number<~@~>Check In<~@~>Check Out<~@~>Total Room Charge<~@~>Total Service Charge<~@~>Total BST Charge<~@~>Total GST Charge<~@~>Total Bank Transfer Charge<~@~>Grand Total";
		array_push($data_array, $data_array_heading);
		...
		if(!empty($booking_room))
		{
			foreach($booking_room as $booking)
			{
				...
				$total_bank_transfer_charge = $setting->currency.round($bank_transfer_charge);
				$grand_total = $setting->currency.round($grand_total_booking_amount);
				$data_line = $booking_number."<~@~>".$room_number."<~@~>".$customer_name."<~@~>".$mobile_number."<~@~>".$guest_category."<~@~>".$address."<~@~>".$id_type."<~@~>".$id_number."<~@~>".$check_in."<~@~>".$check_out."<~@~>".$total_room_charge."<~@~>".$total_service_charge."<~@~>".$total_bst_charge."<~@~>".$total_gst_charge."<~@~>".$total_bank_transfer_charge."<~@~>".$grand_total;
				array_push($data_array, $data_line);
			}
			$data_array_total = "Room Bill Report<~@~><~@~><~@~><~@~><~@~><~@~><~@~><~@~><~@~>All Total<~@~>".$setting->currency.round($all_room_amount)."<~@~>".$setting->currency.round($all_service_tax)."<~@~>".$setting->currency.round($all_bst_tax)."<~@~>".$setting->currency.round($all_gst_tax)."<~@~>".$setting->currency.round($all_bank_transfer_charge)."<~@~>".$setting->currency.round($all_grand_amount);
			array_push($data_array, $data_array_total);
		}
	...
	public function customercsv()
    {
		...
		$setting = $this->Settings->find('all', ['conditions' => []])->first();
		$data_array = array();
		$data_array_heading = "Booking Number<~@~>Room Number<~@~>Customer Name<~@~>Mobile Number<~@~>Guest Category<~@~>Address<~@~>ID Type<~@~>ID Number<~@~>Room Bill<~@~>Food Bill<~@~>Bar Bill<~@~>House Keeping Bill<~@~>Gross Amount<~@~>Service Charge<~@~>BST Charge<~@~>GST Charge<~@~>Bank Transfer Charge<~@~>Grand Total<~@~>Amount Paid<~@~>Outstanding";
		array_push($data_array, $data_array_heading);
		...
		if(!empty($booking_customer))
		{
			foreach($booking_customer as $booking)
			{
				...
				$total_bank_transfer_charge = $setting->currency.round($bank_transfer_charge);
				$grand_total = $setting->currency.round($grand_total_booking_amount);
				$data_line = $booking_number."<~@~>".$room_number."<~@~>".$customer_name."<~@~>".$mobile_number."<~@~>".$guest_category."<~@~>".$address."<~@~>".$id_type."<~@~>".$id_number."<~@~>".$total_room_charge."<~@~>".$total_food_charge."<~@~>".$total_bar_charge."<~@~>".$total_house_keeping_charge."<~@~>".$gross_amount."<~@~>".$total_service_charge."<~@~>".$total_bst_charge."<~@~>".$total_gst_charge."<~@~>".$total_bank_transfer_charge."<~@~>".$grand_total."<~@~>".$setting->currency.round($amount_received)."<~@~>".$setting->currency.round($outstanding);
				array_push($data_array, $data_line);
			}
	...
	public function revenuecsv($start_date = null, $end_date = null)
    {
		...
		$setting = $this->Settings->find('all', ['conditions' => []])->first();
		$data_array = array();
		$data_array_heading = "Booking Number<~@~>Room Number<~@~>Total Room Charge<~@~>Total Food Charge<~@~>Total Bar Charge<~@~>Total House Keeping Charge<~@~>Gross Amount<~@~>Total Service Charge<~@~>Total BST Charge<~@~>Total GST Charge<~@~>Total Bank Transfer Charge<~@~>Grand Total";
		array_push($data_array, $data_array_heading);
		...
		if(!empty($booking_revenue))
		{
			foreach($booking_revenue as $booking)
			{
				...
				$total_bank_transfer_charge = $setting->currency.round($bank_transfer_charge);
				$grand_total = $setting->currency.round($grand_total_booking_amount);
				$data_line = $booking_number."<~@~>".$room_number."<~@~>".$total_room_charge."<~@~>".$total_food_charge."<~@~>".$total_bar_charge."<~@~>".$total_house_keeping_charge."<~@~>".$gross_amount."<~@~>".$total_service_charge."<~@~>".$total_bst_charge."<~@~>".$total_gst_charge."<~@~>".$total_bank_transfer_charge."<~@~>".$grand_total;
				array_push($data_array, $data_line);
			}
	...
	public function outstandingcsv($start_date = null, $end_date = null)
    {
		...
		$setting = $this->Settings->find('all', ['conditions' => []])->first();
		$data_array = array();
		$data_array_heading = "Booking Number<~@~>Room Number<~@~>Customer Name<~@~>Mobile Number<~@~>Check In<~@~>Check Out<~@~>Gross Amount<~@~>Service Charge<~@~>BST Charge<~@~>GST Charge<~@~>Bank Transfer Charge<~@~>Grand Total<~@~>Amount Paid<~@~>Outstanding<~@~>Remarks";
		array_push($data_array, $data_array_heading);
		...
		if(!empty($booking_outstanding))
		{
			foreach($booking_outstanding as $booking)
			{
				...
				if($outstanding > 0)
				{
					...
					$data_line = $booking_number."<~@~>".$room_number."<~@~>".$customer_name."<~@~>".$mobile_number."<~@~>".$check_in."<~@~>".$check_out."<~@~>".$gross_amount."<~@~>".$total_service_charge."<~@~>".$total_bst_charge."<~@~>".$total_gst_charge."<~@~>".$total_bank_transfer_charge."<~@~>".$grand_total."<~@~>".$amount_received."<~@~>".$outstanding."<~@~>".$booking_verified;
					array_push($data_array, $data_line);
				}
			}
	...
	public function deleteroom($id = null, $booking_id = null, $room_name = null)
    {
		$this->loadModel('Rooms');
		$this->loadModel('BookingRoomDetails');
		if($id=='' || $id==null)
		...
		if(isset($id) && $id!=""){
			$booking_detail_id = base64_decode($id);
		}
		...
		$this->Rooms->updateAll(['room_status' => "A", 'booking_id' => "0"], ['booking_id' => $booking_id, 'room_number' => $room_name]);
		$BookingRoomDetails = $this->BookingRoomDetails->get($booking_detail_id);
		if ($this->BookingRoomDetails->delete($BookingRoomDetails)) {
            $this->Flash->success(__('The room has been deleted successfully.'));
        } else {
            $this->Flash->error(__('The room could not be deleted. Please, try again.'));
        }

        return $this->redirect(['action' => 'edit', $booking_id, "deleteRoom"]);
	}
